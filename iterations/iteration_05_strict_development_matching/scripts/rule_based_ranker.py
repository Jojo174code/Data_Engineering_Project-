...
def development_signal(match_strength: str, inv_strength: str, match_quality: str) -> tuple[str, int]:
    if match_quality == 'Invalid Weak Match':
        return 'Low', 1
...
def crime_risk_estimate(address: str) -> tuple[str, int]:
    return ('Unknown', 5) if looks_residential_address(address) else ('Unknown', 4)
...
def main():
    ...
    stale_cols = [
        'development_match_strength', 'development_match_quality', 'development_match_warning', 'matched_development_area',
        'matched_project_or_area_name', 'matched_project_type', 'matched_investment_signal_strength',
        'development_source_url', 'development_source_title', 'development_summary', 'development_match_reason',
        'development_confidence', 'previous_development_match_strength', 'rule_based_score', 'rule_based_category',
        'bid_price_signal', 'address_quality_signal', 'property_clarity_signal', 'development_signal',
        'surrounding_value_signal', 'crime_risk_estimate', 'data_confidence', 'rule_based_reasoning', 'key_rule_based_risks'
    ]
    ...
    def score_row(row):
        ...
        dev_signal, dev_score = development_signal(clean_text(row['development_match_strength']), clean_text(row['matched_investment_signal_strength']), clean_text(row['development_match_quality']))
        ...
        score = round(
            bid_score * 0.20 + addr_score * 0.15 + clarity_score * 0.15 + dev_score * 0.20 + surround_score * 0.15 + crime_score * 0.05 + data_score * 0.10,
            1,
        )
        ...
        weak_dev = clean_text(row['development_match_quality']) == 'Invalid Weak Match'
        ...
        elif bid_signal == 'Strong' and addr_signal == 'Complete' and clarity_signal == 'Strong' and data_confidence in {'Medium', 'High'} and dev_signal == 'High' and not weak_dev and not red_flag:
            category = 'Good Investment'
        elif ((3000 <= bid <= 5500 and addr_signal != 'Missing' and clean_text(row['parcel_id']) != 'Unknown' and data_confidence in {'Medium', 'High'} and not red_flag) or (dev_signal in {'High', 'Medium'} and bid <= 6500 and data_confidence in {'Medium', 'High'} and not weak_dev)):
        ...
        reasoning = (
            ...
            f"strict development match {row['development_match_strength']}, match quality {row['development_match_quality']}, development signal {dev_signal}, data confidence {data_confidence}."
        )
        risks = []
        ...
        if clean_text(row['development_match_strength']) in {'Unknown', 'No Clear Match'}:
            risks.append('no clear strict development overlap')
        if weak_dev:
            risks.append('weak generic development match removed')
