...
SYSTEM_MSG = 'You are a grounded real-estate auction screening assistant. Return valid JSON only. Use only the provided property data and cited development intelligence. Do not invent facts. Do not treat weak or invalid development matches as positive signals. Do not provide generic real estate advice.'
...
Strict Development Matching:
Development Match Strength: {development_match_strength}
Development Match Quality: {development_match_quality}
Development Match Warning: {development_match_warning}
...
Rules:
1. Do not classify as Good Investment if development_match_quality is Invalid Weak Match or if development_match_strength is No Clear Match or Unknown.
2. Treat development_match_warning as a negative signal, not a positive one.
3. Set development_opportunity_flag=true only if there is Strong Match or Possible Match and the match quality is not Invalid Weak Match.
4. Be specific. Mention bid cost, address or parcel quality, and development signal only if the strict match is credible.
5. Do not invent external facts.
6. Return JSON only.
"""
...
def grounded_enough(reasoning: str, row: pd.Series) -> bool:
    ...
    candidates = [
        ...
        clean_text(row['development_match_quality']).lower(),
    ...
    needs_dev = clean_text(row['development_match_strength']) in {'Strong Match', 'Possible Match'} and clean_text(row['development_match_quality']) != 'Invalid Weak Match'
    has_dev_ref = any(clean_text(row[field]).lower() in text for field in ['development_match_strength', 'development_match_quality', 'matched_development_area', 'matched_project_or_area_name'] if clean_text(row[field]))
    return hits >= 3 and ((not needs_dev) or has_dev_ref)
...
def normalize_category(cat: str, row: pd.Series, score: float) -> str:
    ...
    if cat in ALLOWED_CATEGORIES:
        ...
        if lowered == 'needs_further_review':
            normalized = 'Strong Manual Review Candidate' if score >= 5 and row['data_confidence'] in {'Medium', 'High'} else 'Mid Investment'
        elif lowered == 'insufficient_information':
            normalized = 'Mid Investment' if row['data_confidence'] != 'Low' else 'Bad Investment'
        else:
            normalized = row['rule_based_category']
    if clean_text(row['development_match_quality']) == 'Invalid Weak Match' and normalized == 'Good Investment':
        return 'Strong Manual Review Candidate' if row['rule_based_category'] == 'Strong Manual Review Candidate' else 'Mid Investment'
    if clean_text(row['development_match_strength']) in {'No Clear Match', 'Unknown'} and normalized == 'Good Investment':
        return row['rule_based_category'] if row['rule_based_category'] != 'Good Investment' else 'Strong Manual Review Candidate'
    return normalized
...
def normalize_record(data: dict, row: pd.Series) -> dict:
    ...
    credible_dev = clean_text(row['development_match_strength']) in {'Strong Match', 'Possible Match'} and clean_text(row['development_match_quality']) != 'Invalid Weak Match'
    dev_flag = bool(data.get('development_opportunity_flag', credible_dev)) and credible_dev
    dev_reason = clean_text(data.get('development_opportunity_reason', row['development_match_reason'])) if dev_flag else clean_text(row['development_match_warning'] or row['development_match_reason'])
    return {
        ...
        'next_due_diligence_step': clean_text(data.get('next_due_diligence_step', 'Drive-by, title review, and strict location verification')),
...
def fallback_record(row: pd.Series, reason: str) -> dict:
    ...
    return {
        ...
        'development_opportunity_flag': credible_dev,
        'development_opportunity_reason': row['development_match_reason'] if credible_dev else clean_text(row['development_match_warning'] or row['development_match_reason']),
        'reasoning_summary': 'AI review failed validation; fallback rule-based review used.',
        'key_risks': row['key_rule_based_risks'],
        'missing_information': reason,
        'next_due_diligence_step': 'Drive-by, title review, and strict location verification',
...
def main():
    ...
    row_dicts = list(df.to_dict('records'))
    results = [None] * len(row_dicts)
    failures = 0

    max_workers = min(8, max(2, (os.cpu_count() or 4)))
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [executor.submit(review_row, idx, row_dict, client, model) for idx, row_dict in enumerate(row_dicts)]
        completed = 0
        for future in as_completed(futures):
            ...
            if error:
                failures += 1
            if completed % 10 == 0 or completed == len(row_dicts):
                msg = f'Reviewed {completed}/{len(row_dicts)} properties (failures={failures})'
