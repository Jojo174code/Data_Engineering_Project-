...
def rule_based_result(row: pd.Series) -> dict:
    ...
    summary = (
        ...
        f"max bid {row.get('Max Bid')}, grade {row.get('Grade')}, color {row.get('Color')}, crime risk {crime}, and economic signal {econ}."
    )
    risks = []
